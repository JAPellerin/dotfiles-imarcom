...
: >"$CALLS"
assert_ok "réussit sans rien faire" install_font "$ZIP_URL" "Police Test"
...
assert_ok "dossier choisi : famille sans espaces" test -d "$FONTS_DIR/PoliceTest"

printf '%s\n' "== dossier explicite =="
: >"$FAMILIES"
assert_ok "réussit" install_font "$ZIP_URL" "Police Test" MonDossier
assert_file "copié dans le dossier demandé" "$FONTS_DIR/MonDossier/PoliceTest-Regular.ttf"

printf '%s\n' "== échecs : aucun dossier laissé =="
: >"$CALLS"; : >"$FAMILIES"
assert_fail "URL injoignable → échec" install_font "file://$TEST_TMP/nexiste-pas.zip" "Autre Police"
assert_fail "aucun dossier de police" test -e "$FONTS_DIR/AutrePolice"
out=$(install_font "file://$TEST_TMP/casse.zip" "Autre Police" 2>&1)
assert_contains "archive illisible → l'erreur nomme l'URL" "$out" "casse.zip"
assert_fail "aucun dossier de police" test -e "$FONTS_DIR/AutrePolice"
out=$(install_font "file://$TEST_TMP/sans-police.zip" "Autre Police" 2>&1)
