...
| Fichier | Cible | Rôle |
|---|---|---|
| `chromium-1password.json` | `/etc/brave/policies/managed/1password.json`, `/etc/opt/chrome/policies/managed/1password.json` | Stratégie d'entreprise Chromium : extension 1Password (`aeblfdkhhhdcdjpifhhbdiojplfjncoa`, Chrome Web Store) installée en `normal_installed` (activée, désactivable). Doc : https://chromeenterprise.google/policies/#ExtensionSettings |
| `firefox-policies.json` | `/etc/firefox/policies/policies.json` | Idem pour Firefox (`{d634138d-c276-4fc8-924b-40a0ea21d284}`, addons.mozilla.org) ; le module y ajoute `Preferences.intl.locale.requested` quand `firefox-l10n-fr` est installé. Doc : https://mozilla.github.io/policy-templates/ |
