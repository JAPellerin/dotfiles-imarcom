...
6. **Vague 1** — `cli-tools` — **fait** (22 sept 2026, validé dans la WSL : ripgrep 15.1.0, fd-find 10.3.0, fzf 0.67.0, bat 0.25.0, zoxide 0.9.8, lazygit 0.57.0, postgresql-client 18, tous depuis les dépôts Ubuntu) ; `terminal` — **fait** (22 sept 2026, validé en VM : Ghostty 1.3.0 des dépôts Ubuntu, police MesloLGS NF par `install_font` depuis les quatre fichiers de Powerlevel10k, terminal par défaut par l'alternative Debian **et** `~/.config/xdg-terminals.list` — sur Ubuntu 26.04 GNOME délègue à `xdg-terminal-exec`) ; `docker`
