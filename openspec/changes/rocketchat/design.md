...
### D5. Tests
`tests/test-rocketchat.sh` : `HOME` et racine (`ROCKETCHAT_ROOT`) isolés, doublures `dpkg-query`, `github_release_asset_url`, `apt_install_deb_url` (comme `obsidian`) et `run_sudo` (comme `claude-desktop`) ; fonctions appelées **par `module_call`**. Cas : première application (motif `-linux-amd64\.deb$`, paquet `rocketchat`, fichier copié identique au dépôt, étape de connexion) ; `config/rocketchat/servers.json` est un JSON valide qui désigne `https://rocketchat.imarcom.net` (`jq`) ; déjà installé → aucun appel au helper ; fichier retiré ou différent → `module_check` à faire, `module_configure` le réécrit ; fichier utilisateur `~/.config/Rocket.Chat/servers.json` supprimé par le client → toujours « déjà fait » ; écriture système en échec → `module_configure` échoue ; profil AppArmor (D6) : copié et chargé (`apparmor_parser -r` sur la cible), attaché à `rocketchat-desktop.bin` avec `userns`, relance sans changement → aucun chargement, retiré ou différent → à faire puis réécrit et rechargé, chargement en échec → module en échec, fichier retiré, à faire ; recherche ou installation en échec → aucune étape ; `module_check` sur chaque condition.
...
- [Une version du `.deb` installe enfin son propre profil sous le même nom, ou Ubuntu en ajoute un pour Rocket.Chat] → contenu différent : `module_check` repasse « à faire » et la relance remet le profil du dépôt ; deux profils attachés au même binaire se signalent au chargement. À revoir alors : retirer D6 si le profil d'upstream suffit.
...
Aucune. Retour arrière : `apt remove rocketchat`, `sudo rm /opt/Rocket.Chat/resources/servers.json`, `sudo apparmor_parser -R /etc/apparmor.d/rocketchat-desktop && sudo rm /etc/apparmor.d/rocketchat-desktop`.
