...
- [x] 1.1 `modules/65-vpn.sh` (`MODULE_GROUP=apps`, `MODULE_DEPS="base"`, `MODULE_NEEDS_GUI=1`) — en-tête (configuration reportée, décision du 23 sept 2026), `module_install` (D1), `module_check` et `module_configure` avec le constat de profil partagé (D2) ; vérifier `shellcheck` propre et `./setup.sh --list` → « non disponible ici » dans la WSL
- [x] 1.2 `tests/test-vpn.sh` : les cas de D3 ; vérifier `bash tests/run-all.sh` vert et la ligne `shellcheck` de `CLAUDE.md` propre
