...
| Trap EXIT d'un sous-shell `( … )` sur Ctrl-C (essai du 24 sept 2026, bash) | exécuté, que `SIGINT` vise le sous-shell seul ou son groupe de processus : un nettoyage enregistré par `add_cleanup` dans `module_call` s'exécute à l'interruption |
...
Délais : `CONNEXION_WAIT_SECONDS` (300) et `CONNEXION_WAIT_INTERVAL` (2), surchargeables (tests), qui remplacent `NAV_WAIT_*`.
...
6. Boucle : `ui_wait "<libellé> : en attente de la connexion" …` ; réussite → `wl-copy --clear`, `log_ok`, retour 0 ; délai → `ui_choose` « Continuer d'attendre » / « Passer (étape manuelle) » ; « Passer » (ou choix annulé) → vidage, `manual_step`, retour 0.
...
### D4. `open_detached <commande…>`
Commande introuvable (`command -v`) → `log_warn "Impossible de lancer <commande> : l'ouvrir à la main."`, retour 0. Sinon : trace `[hh:mm:ss] $ <commande> (détaché)` au journal, `setsid -f "$@" >/dev/null 2>&1 </dev/null` — l'application n'hérite ni du terminal ni du journal (elle y écrirait sinon ses propres traces tant qu'elle tourne, vu en VM avec 1Password) ; échec du lancement → même avertissement. Toujours 0.
`1password` : `_op_open_settings` garde sa vérification de `xdg-open` (message propre au module) et sa pause entre deux pages, et lance chaque page par `open_detached xdg-open onepassword://settings/<page>`. `navigateur` : `_nav_open_brave` garde le fichier `First Run` et appelle `open_detached brave-browser --no-first-run`.
