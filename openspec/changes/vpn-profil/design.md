...
| Élément `op://Imarcom/VPN` | identifiant (`username`) et mot de passe **fixe** (`password`) ; fichier joint lisible par `op read op://Imarcom/VPN/jpellerin.ovpn` (référence d'un fichier par son nom) — vérifié (tâche 0.1) |
...
| **À relever (tâche 0.3)** | `nmcli connection delete uuid …` sous netplan : YAML de `/etc/netplan/`, fichier rendu sous `/run` et `.pem` retirés ou non ; sortie exacte de `nmcli --show-secrets -g vpn.secrets connection show uuid …` **en utilisateur** depuis la session graphique (le relevé 0.2 a lu le secret dans le YAML par sudo) ; sortie de `nmcli -t -f PERMISSION,VALUE general permissions` dans la session graphique et par SSH |
...
5. Contrôle, tout lu dans des variables, rien affiché : `nmcli -g vpn.data connection show uuid <uuid>` contient `username = <identifiant>` et `password-flags = 0` ; `nmcli --show-secrets -g vpn.secrets connection show uuid <uuid>` donne `password = <valeur>` et **la valeur, désechappée selon le format de `-g` (relevé 0.3), est égale au mot de passe lu dans 1Password** — une valeur vide, `--` (secret illisible, relevé par SSH) ou abîmée par l'échappement échoue.
...
- [`nmcli connection delete` qui laisserait le YAML de netplan] → relevé à la tâche 0.3 avant le code ; sinon D6 le complète.
- [Format de `--show-secrets -g`] → relevé à la tâche 0.3 ; le contrôle en dépend.
