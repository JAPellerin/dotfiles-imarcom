...
| Éditeur de `nmcli` (VM, 25 sept 2026) | **répète sur sa sortie chaque commande reçue**, mot de passe compris (`nmcli> set vpn.secrets password = …`) ; `set vpn.secrets password = ab\,c\\d` enregistre exactement `ab,c\d` (lu dans le YAML de netplan) : échappement de D3 confirmé |
...
- [Éditeur de `nmcli` qui répète le mot de passe] → sortie vers `/dev/null` (D3) ; test : la doublure de `nmcli` répète son entrée standard comme le vrai, et le mot de passe ne doit apparaître ni dans la sortie du module ni dans le journal.
