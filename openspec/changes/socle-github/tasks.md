...
- [ ] 1.1 `lib/github.sh` : `github_release_asset_url` selon D1 à D4, en-tête dans le style des autres `lib/*.sh` ; chargé par `setup.sh` après `lib/apt.sh` ; vérifier `shellcheck lib/github.sh setup.sh` propre et, en réel, `github_release_asset_url obsidianmd/obsidian-releases '_amd64\.deb$'` → l'URL du `.deb` de v1.13.7 (ou plus récent), pas l'`.apk` de v1.13.8
- [ ] 1.2 `tests/test-github.sh` : les cas de D5 ; vérifier `bash tests/test-github.sh` vert et qu'une mutation (retirer le filtre `prerelease`, ou prendre `/releases/latest`) le fait échouer
