...
- [ ] 5.4 Test de bout en bout dans la VM : `curl -fsSL <url-raw>/bootstrap.sh | bash` → menu → `base` + `1password` (app + CLI, intégration, agent SSH) ; vérifier le résumé final, `op whoami`, `source ~/.commonrc && ssh-add -l` (le chargement automatique de `.commonrc` arrive avec le module `shell`), puis restaurer le snapshot et relancer pour vérifier l'idempotence (« déjà fait » partout)
