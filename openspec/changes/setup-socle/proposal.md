...
- **Runner `setup.sh`** : point d'entrée unique. Sans argument, menu interactif `gum` listant les modules avec leur état ; avec argument, exécution d'un ou plusieurs modules par nom (`setup.sh git node`), plus `--all`, `--list`. Tourne en utilisateur, demande `sudo` une seule fois (keepalive). Résumé final : fait / sauté / échoué + étapes manuelles restantes.
- **Contrat de module** : un fichier par module dans `modules/`, déclarant `MODULE_NAME`, `MODULE_DESC`, `MODULE_DEPS` et les fonctions `module_check`, `module_install`, `module_configure`. Le runner résout les dépendances et saute ce qui est déjà fait (idempotence).
...
- **Mise à jour de `CLAUDE.md`** avec les décisions de conception.
...
- Nouveaux fichiers : `bootstrap.sh`, `setup.sh`, `lib/*.sh`, `modules/00-base.sh`, `modules/10-1password.sh` ; `CLAUDE.md` mis à jour.
