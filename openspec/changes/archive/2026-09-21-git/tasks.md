...
- [x] 2.4 Exécution réelle dans la WSL : `./setup.sh git` → dépôt `gh` ajouté, `gh` et `delta` installés, `~/.gitconfig` lié (`.bak`), `known_hosts` complété, clé existante non touchée (message), `gh auth login` réel réussi ; `git diff` dans le dépôt passe par delta ; relance → « déjà fait » — fait le 18 sept 2026 : delta 0.18.2, gh 2.101.0 (dépôt officiel), lien + .bak, 3 clés github.com, clé WSL intacte sans lecture op, gh connecté (device flow ~1 min), `--list` → déjà fait
...
- [x] 3.2 VM (snapshot « vierge ») : bootstrap → `base` + `1password` + `shell` + `git` → dans un nouveau terminal : `ssh -T git@github.com` (via l'agent, sans fichier de clé dans `~/.ssh`), `git clone git@github.com:JAPellerin/dotfiles-imarcom.git /tmp/t`, `git -C /tmp/t diff HEAD~1` affiché par delta, `gh auth status` ; relance → « déjà fait » partout — fait le 18 sept 2026 : 4 modules en 7 min, aucun fichier de clé (agent), `ssh -T` authentifié, clone SSH, `gh` connecté (jeton dans le trousseau GNOME, `gh auth token` OK même par SSH), `--list` → déjà fait ; observé : l'app 1Password écrit elle-même `~/.ssh/config` (`Host * IdentityAgent ~/.1password/agent.sock`) à l'activation de l'agent
