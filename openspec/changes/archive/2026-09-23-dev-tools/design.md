...
### D4. twg : `--yes --skip-login --skip-skills`
`bash install --yes --skip-login --skip-skills` : l'installateur ne lance pas le navigateur, n'installe pas ses skills dans les agents détectés (Claude Code…), et **accepte les conditions d'utilisation d'Atlassian sans interaction**.
**Correction du 23 sept 2026 (premier passage en VM, tâche 2.2).** Avec `--skip-login` seul, le module est resté bloqué plus d'une heure : l'installateur lance `twg setup finalize`, qui lance `twg consent` — une question « I agree and want to continue ? y / N ». L'installateur la pose **sur `/dev/tty`**, qu'il ouvre lui-même (« Pipe installs consume stdin; use the controlling TTY »), ce qui contourne l'entrée fermée par `run` ; la question s'affichait dans le terminal pendant que sa sortie partait au journal, sans que rien n'indique d'y répondre. `twg setup finalize --yes` (« Run non-interactively ») l'évite, mais l'installateur ne le lui transmet que si `--yes`, `--skip-login` **et** `--skip-skills` sont tous trois donnés (ligne 708 du script). Accepter les conditions par le script est la décision de l'utilisateur (23 sept 2026) ; il les avait déjà acceptées dans la WSL (`~/.config/twg/consent.json`, 11 sept 2026). Les skills restent possibles après coup par `twg skills`. `twg` se met à jour lui-même (`twg update`, politique de version minimale côté serveur) : le module ne s'en occupe pas.
