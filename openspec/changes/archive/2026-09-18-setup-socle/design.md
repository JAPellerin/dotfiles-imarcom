...
Bash est le langage naturel des commandes d'installation (apt, curl, gpg) et tourne sans rien sur une machine vierge ; `gum` apporte les menus, confirmations et saisies dignes de `create-vite` (le spinner `ui_spin` est une boucle Bash : `gum spin` ne peut lancer qu'un exécutable, pas les fonctions `run`/`run_sudo` qui journalisent — décidé le 17 sept). Alternatives : Node + `@clack/prompts` (belle UX mais Node à installer d'abord, double couche), Python + `questionary` (idem), `whiptail` (aucune dépendance mais UX datée). `gum` 0.17 est dans les dépôts Ubuntu 26.04 : `apt install gum`, sans dépôt Charm — plus simple et cohérent avec « apt d'abord ».
...
Le dépôt apt `downloads.1password.com/linux/debian/<arch>` sert les deux paquets (`1password`, `1password-cli`), avec la politique `debsig` demandée par la doc officielle. L'app n'est installée que si `has_gui` est vrai (D9). Connexion : intégration app → CLI si l'app est là (déverrouillage par l'app, agent SSH), sinon `op account add` interactif. La session issue de `op signin` est exportée dans l'environnement du runner pour les modules suivants (`eval "$(op signin)"`).
Complément du 18 sept 2026 : les trois réglages de l'app nécessaires (« Unlock using system authentication », « Integrate with 1Password CLI », « Use the SSH agent ») n'ont **aucune commande ni fichier de configuration** — la doc d'intégration ne prévoit que l'interface, et l'app signe ses réglages sensibles (toute édition de `~/.config/1Password/settings/settings.json` est remise à zéro). L'app et le CLI ont des stockages de compte séparés ; intégration active, le CLI délègue à l'app et `op account add` devient inutile. Le module réduit donc la friction autour des clics restants : ouverture de l'app sur ses pages de réglages, attente du socket de l'agent SSH, un seul `op signin` — voir `openspec/changes/archive/2026-09-18-1password-integration-app/design.md` (D1–D6).
...
- [Intégration app → CLI de 1Password impossible sans clic dans l'interface (réglages signés, pas d'API)] → le module ouvre l'app directement sur ses pages de réglages, affiche la consigne et attend seul que l'agent SSH apparaisse, puis vérifie la session ; repli `op account add` en terminal. Plus d'étape manuelle différée (change `1password-integration-app`).
