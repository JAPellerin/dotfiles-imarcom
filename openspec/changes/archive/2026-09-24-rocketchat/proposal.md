...
- **Module `rocketchat`** (`modules/61-rocketchat.sh`, groupe `apps`, dépend de `base`, **session graphique requise**) :
  ...
  - **profil AppArmor** (ajouté le 24 sept 2026, après le test en VM) : sans lui, Ubuntu 26.04 refuse au client les espaces de noms utilisateur et il plante au lancement ; le paquet n'installe pas le sien (et le vise mal), et son `postrm` supprime `/etc/apparmor.d/rocketchat-desktop` à chaque mise à jour : `config/rocketchat/apparmor-profile` copié vers `/etc/apparmor.d/opt.Rocket.Chat.rocketchat-desktop.bin` et chargé par `apparmor_parser -r` ;
...
- Écritures système (avec `sudo`) : paquet `rocketchat` (`/opt/Rocket.Chat`, `rocketchat-desktop.desktop`) ; `/opt/Rocket.Chat/resources/servers.json` (copie du dépôt, déclarée par aucun paquet) ; `/etc/apparmor.d/opt.Rocket.Chat.rocketchat-desktop.bin` (copie du dépôt), chargé dans le noyau.
