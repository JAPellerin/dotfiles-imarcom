...
La sélection `code code/add-microsoft-repo boolean false` est écrite dans un fichier temporaire (nettoyé par `add_cleanup`), puis passée **en argument** : `run_sudo debconf-set-selections <fichier>`, avant `apt_install code`. Pas de tube `echo … | run_sudo …` : `run` branche l'entrée de la commande sur `/dev/null`, la sélection n'arriverait jamais à `debconf-set-selections`, qui réussirait sans rien régler — et le paquet déclarerait son dépôt. La doc montre la même clé réglée à `true` pour l'effet inverse ; `false` laisse la déclaration du dépôt au socle. Idempotent (réécrire la même sélection est sans effet). À constater en VM (tâche 2.1) : après installation, `ls /etc/apt/sources.list.d/` ne montre qu'un `vscode.sources`, identique à ce qu'écrit `apt_add_repo`, et une relance ne le réécrit pas.
...
### D6b. Connexion Jira et Bitbucket : étape manuelle à la première installation
Demande de l'utilisateur (23 sept 2026) : faire la connexion de l'extension Atlassian par le script. **Non automatisable** : `atlassian.atlascode` se connecte par OAuth dans le navigateur, depuis son propre écran, et range ses jetons dans le stockage de secrets de VS Code (gnome-keyring, chiffré) — aucune commande ni fichier documenté pour y déposer une connexion ; écrire dans le trousseau à la place de l'éditeur dépendrait d'un format interne. Même limite que les réglages signés de 1Password.
À la place : **dans `module_install`**, si `code` était absent avant son passage et que `apt_install` réussit, `manual_step "Connecter Jira et Bitbucket dans VS Code : extension Atlassian (barre latérale) → se connecter, dans le navigateur."`. Pas d'étape quand VS Code était déjà là (relance pour une extension ajoutée à la liste) : la connexion est faite une fois, et son état n'est pas lisible — même principe que `claude-desktop` D5.
**Correction du 23 sept 2026 (contre-vérification).** La première version notait l'absence de `code` dans une variable de `module_install` et déclarait l'étape dans `module_configure` : le runner lance les deux fonctions dans des sous-shells distincts (`module_call`, `lib/module.sh`), la variable y valait toujours 0 et l'étape n'aurait jamais paru. Le test appelait les deux fonctions dans le même shell et ne le voyait pas ; il les appelle désormais chacune dans un sous-shell, comme le runner.

### D6. Tests
`tests/test-vscode.sh` (nouveau) : doublures `dpkg-query`, `run_sudo` (journalise ; simule `apt-get install` ; pour `debconf-set-selections`, copie le **fichier passé en argument** — la sélection vérifiée est donc celle qui atteint réellement la commande, et une doublure qui lirait son entrée standard masquerait le défaut que D2 évite), `apt_add_repo` (compte), faux `code` (`--list-extensions` lit un fichier, `--install-extension` l'ajoute ou échoue pour un identifiant marqué). Cas : première application (sélection debconf posée **avant** `apt-get install`, fichier contenant exactement `code code/add-microsoft-repo boolean false`, dépôt, paquets `code` et `gnome-keyring`, 18 extensions) ; réexécution → aucun appel ; extensions partielles → seules les manquantes ; casse différente (`Anthropic.Claude-Code`) → tenue pour installée ; extension hors liste → jamais retirée ; échec d'une extension → module en échec nommant l'identifiant ; commentaires et lignes vides ignorés ; `module_check` sur chaque condition ; étape de connexion déclarée quand `code` vient d'être installé, absente quand il l'était déjà — `module_install` et `module_configure` appelés chacun dans un sous-shell, comme par le runner. Plus : `config/vscode/extensions.txt` contient exactement les 18 identifiants décidés.
