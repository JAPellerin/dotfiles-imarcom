...
- [x] 4.2 Exécution réelle dans la WSL : `git pull` puis, dans un nouveau terminal, vérifier que `~/.commonrc` (lien vers le dépôt) charge bien un fragment d'essai déposé à la main dans `~/.commonrc.d`, puis le retirer ; `./setup.sh shell` → « déjà fait » (le module n'a pas changé) ; `./setup.sh --list` inchangé — fait le 22 sept 2026 : pas de `git pull` (commits locaux, et `~/.commonrc` est un lien vers l'arbre de travail : la boucle est active sans rien tirer) ; fragment lu par `bash -ic` et `zsh -ic`, puis dossier retiré → aucune variable résiduelle et aucun message au démarrage des deux shells ; `./setup.sh shell` lancé par l'utilisateur dans un terminal ordinaire → « déjà fait » ; `--list` identique. Constat au passage : `setup.sh` appelle `sudo_keepalive` sans condition (`setup.sh:317`), donc un module déjà fait demande quand même le mot de passe — hors périmètre de ce change (touche `setup-runner`).
