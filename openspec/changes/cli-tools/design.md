...
Garde-fou (**confirmé par l'utilisateur le 22 sept 2026**, contre la sauvegarde en `.bak` du reste du projet) : si `~/.local/bin/bat` existe et n'est pas le lien attendu, le module le **laisse intact** et le signale (`log_warn`). Le poste peut avoir un vrai binaire installé à la main ; l'écraser serait une perte silencieuse. `module_check` renvoie alors « à faire », ce qui est exact : la configuration attendue n'est pas en place. **Correction du 22 sept 2026 (contre-vérification)** : le `log_warn` seul ne suffisait pas. `module_configure` renvoyant 0, le runner marquait le module « fait » dans le résumé final, pendant que `module_check` le disait « à faire » — l'avertissement, lui, avait défilé. Le module déclare donc aussi une étape manuelle (`manual_step`), que le runner reprend dans « Étapes manuelles restantes », comme `navigateur` le fait pour Brave Sync.

**Portée du garde-fou — tranchée par l'utilisateur le 22 sept 2026.** La condition `[[ -e $target || -L $target ]]` couvre trois situations : (a) un vrai fichier posé à la main (un `bat` compilé ou téléchargé) — le cas qui a motivé le garde-fou ; (b) un lien vers autre chose (`/opt/bat/bat`, un autre gestionnaire de paquets) ; (c) un lien **posé par le module lui-même** et devenu périmé, si Ubuntu déplace `/usr/bin/batcat` ou si le paquet est purgé puis réinstallé ailleurs. Le garde-fou vaut pour **tout ce qui occupe le chemin, lien compris** : dans le cas (c) le module ne répare donc pas son propre lien, et l'étape manuelle demande de le retirer.

Alternative gardée en réserve (**option B**, non retenue pour l'instant) : n'écraser qu'un **lien symbolique** et ne protéger qu'un vrai fichier ou dossier. Remplacer un lien ne détruit rien — la cible existe toujours, on ne perd que la destination précédente, que le journal peut consigner — et les cas (b) et (c) se répareraient tout seuls. Écartée parce que le seul vrai défaut du statu quo (l'écart passait inaperçu) est désormais fermé par l'étape manuelle, et parce que (c) suppose qu'Ubuntu déplace un binaire : rare, contre une surcharge silencieuse d'un `bat` que l'utilisateur aurait délibérément installé, qui est un scénario qu'il peut réellement provoquer. **À réévaluer si le cas (c) se présente pour de vrai** — c'est trois lignes dans `_cli_tools_rename` et un cas de test.
...
### D7. Dépendance au module `shell` (ajoutée le 22 sept 2026, contre-vérification)
`MODULE_DEPS="base shell"`, et non `"base"` seul. Le module dépose un fragment dans `~/.commonrc.d/`, or ce dossier n'est lu que par `~/.commonrc`, que seul `shell` déploie ; et les initialisations de `fzf` et `zoxide` vivent dans `zshrc` et `bashrc-extra.sh`, fichiers de `shell`. Sans lui, `./setup.sh cli-tools` installe les outils, crée les liens, et `module_check` répond « déjà fait » — alors que `FZF_DEFAULT_COMMAND` reste vide et que ni `Ctrl-R` ni `z` n'existent : deux des trois comportements de la spec sont inertes. En `--all` l'ordre des préfixes (20 avant 22) masquait le problème ; le lancement isolé d'un module est une contrainte du projet, pas un cas limite.
Seuls les liens `bat` et `fd` survivent sans `shell`, parce que le `~/.profile` par défaut d'Ubuntu ajoute `~/.local/bin` au `PATH` — pas grâce à `commonrc`, contrairement à ce que disait l'en-tête du module.

## Risks / Trade-offs

- [Un `~/.local/bin/bat` laissé intact bloque `module_check` à « à faire » indéfiniment] → c'est voulu, et visible là où on le regarde : l'étape manuelle nomme le fichier et l'action à faire (le retirer, puis relancer le module) dans le résumé final, pas seulement dans le journal. Écraser silencieusement serait pire.
