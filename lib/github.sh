...
# Dépend de lib/core.sh (log_*, add_cleanup) et de jq (module base).
...
# régulière étendue, ancrée par l'appelant : '_amd64\.deb$') dans la plus récente
# des releases publiées qui en contient un. Brouillons et préversions écartés.
...
github_release_asset_url() {
  local repo=${1:-} pattern=${2:-} tmp url
  [[ -n $repo && -n $pattern ]] \
    || { log_error "github_release_asset_url : arguments manquants (dépôt, motif)"; return 1; }
  # Réponse dans un temporaire, pas de tube curl | jq : un échec de curl (réseau,
  # limite de débit, dépôt inconnu) doit se distinguer d'une liste sans fichier.
  # Hors `run` : la sortie est le document JSON lui-même.
  tmp=$(mktemp -t "github-releases.XXXXXX") || return 1
  add_cleanup "rm -f '$tmp'"
  curl -fsSL --retry 2 "$GITHUB_API_URL/repos/$repo/releases?per_page=20" -o "$tmp" 2>/dev/null \
    || { log_error "API GitHub injoignable ou en erreur pour $repo"; return 1; }
  url=$(jq -r --arg re "$pattern" \
    '[.[] | select((.draft or .prerelease) | not) | .assets[] | select(.name | test($re)) | .browser_download_url][0] // empty' \
    "$tmp" 2>/dev/null) \
    || { log_error "Réponse illisible de l'API GitHub pour $repo"; return 1; }
  [[ -n $url ]] \
