...
- `shellcheck setup.sh bootstrap.sh lib/*.sh modules/*.sh tests/*.sh tests/fixtures/*/*.sh config/shell/commonrc config/shell/bashrc-extra.sh` — lint (aucun avertissement toléré)
...
- La config shell commune bash/zsh vit dans `~/.commonrc` (POSIX uniquement), chargé par `.bashrc` et `.zshrc` ; ce qui est propre à un shell reste dans son fichier. Shell cible : zsh + oh-my-zsh + Powerlevel10k. Sources : `config/shell/` (`zshrc`, `commonrc`, `p10k.zsh` liés dans `~` ; `bashrc-extra.sh` chargé par une ligne ajoutée au `.bashrc` d'Ubuntu).
